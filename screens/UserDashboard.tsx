        ...
        if (availableMin > 0) {
            ...
            while (availableMin > 0 && attemptsInDay < maxAttempts) {
                ...
                if (item.simuladoId) {
                    ...
                    while (cursor < tasks.length && (availableMin > 0 || completedGoalIds.includes(tasks[cursor].subGoalId ? `${tasks[cursor].goal.id}:${tasks[cursor].subGoalId}` : tasks[cursor].goal.id))) {
...
const dayItems = (schedule[todayStr] || []).filter(item => {
    const id = item.subGoalId ? `${item.goalId}:${item.subGoalId}` : item.goalId;
    return !updatedUser.progress.completedGoalIds.includes(id);
});
      ...
      if (Math.ceil(nextItem.duration) > remainingMin) {
          ...
          setConfirmModal({
               ...
               onConfirm: () => {
                   setAdvanceMode(true);
                   setConfirmModal(null);
                 // FISCAL: Desliga a antecipação se o lote de hoje acabar
const hoje = getTodayStr();
const itensDeHoje = schedule[hoje] || [];
const concluiuTudo = itensDeHoje.every(item => {
    const idUnico = item.subGoalId ? `${item.goalId}:${item.subGoalId}` : item.goalId;
